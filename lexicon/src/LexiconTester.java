/*
 * This main program is provided to test the implementation of your
 * Lexicon implementation. 
 * 
 * You do not have to modify this class EXCEPT that the declared Lexicon object lex
 * should instantiate a LexiconTrie (commented line 19) and not LexiconOrderedList.
 * (LexiconOrderedList is only here to introduce you to the expected behavior
 * of the Lexicon. It is not an efficient structure to store the words, that
 * is why you are solving the problem using a Trie.)
 * 
 */
...
public class LexiconTester {
	boolean done = false;
	//Lexicon lex = new LexiconOrderedList();
	Lexicon lex = new LexiconTrie();


	/*
	 * LexiconTester allocates the starting empty lexicon and then enters into
	 * the command loop that allows the user to enter commands until finished.
	 *
	 * Note that the syntax and structure for this entire class is a
	 * little odd.  This is mostly because it was translated from C++
	 * code.  Do not get bogged down in the details.
	 */
	public static void main(String s[]) {	
		LexiconTester m = new LexiconTester();
		m.run();
	}
	 ...
	 * that option. 
	 */
	private boolean invokeCmd(String tokens[]) {
	 ...
	 */
	private void testAdd(String commandTokens[]) {
	 ...
	 */
	private void testRemove(String commandTokens[])	{
		String word = commandTokens[1];
	 ...
	 */
	private void testContains(String commandTokens[]) {
	 ...
	 * the lexicon.
	 */
	private void testReadFile(String commandTokens[]) {
	 ...
	 */
	private void testIterator(String unused[]) {
